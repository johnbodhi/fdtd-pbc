...
for ind=1:number_of_sampled_electric_fields 
    ...
    if incident_plane_wave.enable == true
        hold on;
        incident_field = ...
            sampled_electric_fields(ind).incident_field_value(1:time_step);
        plot(sampled_time, incident_field(1:time_step),...
            'r--','linewidth',1.5);
        total_field = incident_field + sampled_value;
        plot(sampled_time, total_field(1:time_step),...
            'k-.','linewidth',1.5);
        legend('scattered electric field', ...
            'incident electric field', 'total electric field');
        hold off;        
    end
    set(gca,'fontsize',12);
    drawnow;
