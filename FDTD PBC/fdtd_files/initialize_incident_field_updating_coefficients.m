% initialice incident field updating coefficients
...
Cexeic= (2*(1-eps_r_x)*eps_0-dt*sigma_e_x)./(2*eps_r_x*eps_0+dt*sigma_e_x);
Cexeip=-(2*(1-eps_r_x)*eps_0+dt*sigma_e_x)./(2*eps_r_x*eps_0+dt*sigma_e_x);

% Coeffiecients updating Ey
Ceyeic= (2*(1-eps_r_y)*eps_0-dt*sigma_e_y)./(2*eps_r_y*eps_0+dt*sigma_e_y);
Ceyeip=-(2*(1-eps_r_y)*eps_0+dt*sigma_e_y)./(2*eps_r_y*eps_0+dt*sigma_e_y);

% Coeffiecients updating Ez
Cezeic= (2*(1-eps_r_z)*eps_0-dt*sigma_e_z)./(2*eps_r_z*eps_0+dt*sigma_e_z);
Cezeip=-(2*(1-eps_r_z)*eps_0+dt*sigma_e_z)./(2*eps_r_z*eps_0+dt*sigma_e_z);
